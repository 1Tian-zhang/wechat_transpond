...
// DeleteDocumentBlocks 删除指定块下的所有子块，但保留引用容器结构
func (s *Service) DeleteDocumentBlocks(documentID string, parentBlockID string) error {
	// 删除指定父块下的所有子块，但保留引用容器
	...
	if err != nil {
		return fmt.Errorf("failed to delete children of block %s: %w", parentBlockID, err)
	}

	if !resp.Success() {
		return fmt.Errorf("failed to delete children of block %s: %s", parentBlockID, resp.CodeError.Error())
	}
...
func (s *Service) SyncChatlog(syncItem *conf.FeishuSyncMapItem, messages []*model.Message) error {
	...
	for _, block := range blocks {
		if block.BlockType != nil && *block.BlockType == 2 && block.Heading2 != nil && block.Heading2.Elements != nil {
	...
	for date, msgs := range dateGroups {
		// 如果该日期已存在，先删除整个日期块及其子块
		if existingBlock, exists := existingDateBlocks[date]; exists {
			// 删除该日期块下的所有子块
			if err := s.DeleteDocumentBlocks(documentID, *existingBlock.BlockId); err != nil {
		...
		if err := s.CreateTextBlockInQuote(documentID, quoteContainerID, allContent.String()); err != nil {
			return fmt.Errorf("failed to create message block: %w", err)
		}
	}
